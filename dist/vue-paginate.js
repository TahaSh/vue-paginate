...
(function (global, factory) {
  ...
  warn = function (msg, vm, type) {
    ...
    if (hasConsole) {
      console[type](
        ("[vue-paginate]: " + msg + " " + (vm ? formatLocation(formatComponentName(vm)) : ''))
      );
    }
  };
  ...
  var Paginate = {
    ...
    mounted: function mounted() {
      if (this.per <= 0) {
        warn(
          ("<paginate name=\"" + (this.name) + "\"> 'per' prop can't be 0 or less."),
          this.parent
        );
      }
      if (!this.parent.paginate[this.name]) {
        warn(
          ("'" + (this.name) + "' is not registered in 'paginate' array."),
          this.parent
        );
        return;
      }
    ...
    methods: {
      ...
      goToPage: function goToPage(page) {
        ...
        if (page > lastPage) {
          warn(
            ("You cannot go to page " + page + ". The last page is " + lastPage + "."),
            this.parent
          );
          return;
        }
  ...
  var LimitedLinksGenerator = function LimitedLinksGenerator(listOfPages, currentPage, limit) {
    ...
    this.currentPage =
      currentPage === this.lastPage ? this.lastPage - 1 : currentPage;
    this.limit = limit;
  };
  ...
  LimitedLinksGenerator.prototype._buildFirstHalf = function _buildFirstHalf () {
    var firstHalf = this._allPagesButLast().slice(
  ...
  var PaginateLinks = {
    ...
    mounted: function mounted() {
      ...
      if (this.simple && this.limit) {
        warn(
          ("<paginate-links for=\"" + (this.for) + "\"> 'simple' and 'limit' props can't be used at the same time. In this case, 'simple' will take precedence, and 'limit' will be ignored."),
          this.parent,
          'warn'
        );
      }
    ...
    methods: {
      updateListOfPages: function updateListOfPages() {
        ...
        if (!this.target) {
          ...
          warn(
            ("<paginate-links for=\"" + (this.for) + "\"> can't be used without its companion <paginate name=\"" + (this.for) + "\">"),
            this.parent
          );
          warn(
  ...
  function getFullLinks(vm, h) {
    ...
    var allLinks = vm.showStepLinks
      ? [stepLinks.prev ].concat( vm.listOfPages, [stepLinks.next])
      : vm.listOfPages;
  ...
  function getLimitedLinks(vm, h) {
    ...
    limitedLinks = vm.showStepLinks
      ? [stepLinks.prev ].concat( limitedLinks, [stepLinks.next])
      : limitedLinks;
  ...
  function getSimpleLinks(vm, h) {
    ...
    var prevLink = h('li', prevListData, [
      h('a', prevData, [vm.$slots.prev ? vm.$slots.prev : LEFT_ARROW]) ]);
    var nextLink = h('li', nextListData, [
  ...
  function getListOfPageNumbers(numberOfPages) {
    ...
    return Array.apply(void 0, { length: numberOfPages }).map(function (val, index) { return index; });
  }
  ...
  function getTargetPageForLink(
    ...
    if (link === prev) {
      ...
      var chunkContent = listOfPages.slice(
        currentChunk * limit,
